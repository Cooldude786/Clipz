...
export class UploadComponent implements OnDestroy {
  ...
  uploadFile() {
    ...
    const clipFileExtension = this.file?.type.split('/').splice(1).join() ?? 'mp4'
    const clipPath = `clips/${clipFileName}.${clipFileExtension}`
    ...
    this.task.snapshotChanges().pipe(
      ...
      next: async (url) => {
        const clip = {
          ...
          fileName: `${clipFileName}.${clipFileExtension}`,
          url,
